...
let currentFileName = 'ascii_art'; // Default filename
const baseFontSize = 7; // Base font size for ASCII art

// ---------------- Helper Functions ----------------

// TODO Custom Char Set
// TODO Center result
...
function createLine(x1, y1, x2, y2, isDarkMode) {
  const line = createSvgElement("line");
  line.setAttribute("x1", x1);
  line.setAttribute("y1", y1);
  line.setAttribute("x2", x2);
  line.setAttribute("y2", y2);
  line.setAttribute("stroke", isDarkMode ? "#eeeeee" : "#000000");
  line.setAttribute("stroke-width", "2");
  return line;
}

function createDotPattern(x, y, w, h, spacing) {
  const g = createSvgElement("g");
  const dotSize = 1;
  for (let dy = 0; dy < h; dy += spacing) {
    for (let dx = 0; dx < w; dx += spacing) {
      const dot = createRect(x + dx, y + dy, dotSize, dotSize);
      g.appendChild(dot);
    }
  }
  return g;
...
function getCharacterElement(char, x, y, charWidth, fontSize, yPos, isDarkMode) {
  const text = createSvgElement("text");
  text.setAttribute("x", x);
  text.setAttribute("y", yPos);
  text.setAttribute("font-family", "monospace");
  text.setAttribute("font-size", fontSize + "px");
  text.setAttribute("fill", isDarkMode ? "#eeeeee" : "#000000");
...
function downloadSVG() {
  ...
  try {
    ...
    console.log('ASCII text loaded, length:', asciiText.length);
    ...
    // Add background
    const background = createRect(0, 0, width, height);
    background.setAttribute("fill", isDarkMode ? "#000000" : "#ffffff");
    svg.appendChild(background);

    // Create container for shapes
    const g = createSvgElement("g");
    g.setAttribute("fill", isDarkMode ? "#eeeeee" : "#000000");
    svg.appendChild(g);

    // Convert characters to shapes
    let yPos = 30;
    lines.forEach((line, lineIndex) => {
      if (line || lineIndex < lines.length - 1) {
        ...
        chars.forEach((char, charIndex) => {
          if (char !== ' ') {
            const x = 20 + charIndex * charWidth;
            const y = yPos - fontSize / 2;
            const element = getCharacterElement(char, x, y, charWidth, fontSize, yPos, isDarkMode);
            if (element) {
    ...
    a.href = url;
    a.download = `${currentFileName}_ascii.svg`;
...
function downloadPNG() {
  ...
  offCanvas.toBlob(function (blob) {
    ...
    a.download = `${currentFileName}_ascii.png`;
    a.click();
...
document.getElementById('upload').addEventListener('change', function (e) {
  ...
  const thumbnail = document.querySelector('.file-thumbnail');

  if (!file) {
    fileInfo.classList.remove('has-file');
    fileName.textContent = '';
    thumbnail.src = '';
    currentFileName = 'ascii_art';
    return;
  }
  ...
  // Save filename without extension for later use
  ...
  // Continue with original reader for ASCII conversion
...
async function showToast(message, isSuccess = true, duration = 2000) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => toast.remove(), 300); // Remove after fade out animation
  }, duration);
}
...
document.getElementById('downloadBtnSvg').addEventListener('click', downloadSVG);
